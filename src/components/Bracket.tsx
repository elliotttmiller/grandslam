...
export function MatchCard({
  ...
  const canSelectMatch = !readOnly && allowChangingPicks !== false && match.player1 && match.player2 && !isByeMatch;
  ...
  const isUserPick = !!match.winnerId && !hasOfficialResult;
  ...
  const renderPlayer = (player: Player | null, isTop: boolean) => {
    ...
    const canSelect = canSelectMatch && !!player && !isQualifier;
    ...
    return (
      <motion.div
          ...
          isWinner
            ? hasOfficialResult
              ? isWinnerIncorrect
                ? "bg-red-600/6 font-semibold text-red-300"
                : "bg-emerald-600/6 font-semibold text-emerald-300"
              : "bg-card/80 font-semibold text-foreground"
            : "",
          isWinner && !hasOfficialResult && !isByeMatch ? "glow-ring" : "",
          isLoser ? "opacity-40" : "",
          canSelect ? "cursor-pointer hover:-translate-y-0.5 hover:shadow-md" : "cursor-default",
          !player ? "text-muted-foreground/40 italic" : ""
        )}
        whileTap={canSelect ? { scale: 0.992 } : {}}
      ...
      >
        <div className="flex items-center gap-3 overflow-hidden min-w-0">
          {player?.seed ? (
            <span className="text-[11px] text-muted-foreground/70 w-5 text-center shrink-0 font-mono leading-none">
              {player.seed}
            </span>
          ) : (
          ...
          <div className="truncate leading-tight">
            <div className={cn(isQualifier && !isWinner ? "text-muted-foreground/60 text-[13px]" : "text-[14px]")}>{player ? player.name : 'TBD'}</div>
            {player?.country && <div className="text-xs text-muted-foreground/60">{player.country}</div>}
          </div>
        </div>
        ...
        {isWinner && (
          <motion.div
            ...
            className={cn("ml-2 shrink-0 rounded-full p-1 flex items-center justify-center", isWinnerIncorrect ? "bg-red-600/10 text-red-400" : hasOfficialResult ? "bg-emerald-600/10 text-emerald-400" : "bg-white/5 text-foreground")}
            aria-hidden="true"
          >
            <Check size={14} />
          </motion.div>
  ...
  return (
    <motion.div
      ...
      animate={justPicked ? { opacity: 1, scale: [1, 1.04, 1] } : { opacity: 1, scale: 1 }}
      transition={justPicked ? { duration: 0.28, ease: 'easeOut' } : { duration: 0.15 }}
          ...
          match.winnerId
            ? isIncorrectPick
              ...
              : hasOfficialResult
                ...
                : "border-border/40 border-l-[3px] border-l-border/20 bg-card/80"
            : "border-border/40 border-l-[3px] border-l-border/20",
          match.winnerId && !hasOfficialResult && !isByeMatch ? "shadow-white/10" : "",
          !match.winnerId ? "group-hover:border-border/70 group-hover:shadow-md" : "",
          justPicked ? (hasOfficialResult ? "shadow-emerald-500/20 shadow-lg" : "shadow-white/20 shadow-lg") : ""
        )}
      >
        <CardContent>
